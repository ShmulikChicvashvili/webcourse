...
public class ServerAsyncCommunication {
	...
	static public void getGradesSheetfromServer(final Context context,
		...
		UGAsync<AccomplishedCourse> a = new UGAsync<AccomplishedCourse>() {
			...
			protected List<AccomplishedCourse> doInBackground(String... params) {
				UGLoginObject loginObject = new UGLoginObject(studentId,
						password);
	 ...
	 * final String studentId, final String password) { UGLoginObject
	 * loginObject = new UGLoginObject(studentId, password);
	 * List<AccomplishedCourse> result =
	...
	static public void getCalendarEventsFromServer(final Context context) {
		...
		UGAsync<AcademicCalendarEvent> a = new UGAsync<AcademicCalendarEvent>() {
			...
			protected List<AcademicCalendarEvent> doInBackground(
					String... params) {
				List<AcademicCalendarEvent> l = UgFactory.getUgEvent()
	 ...
	 * currentLoginObject =
	 * UGDatabase.getInstance(mainActivity).getCurrentLoginObject();
	 * //List<Exam> l =
	 * UgFactory.getUgExam().getStudentExams(currentLoginObject, semester);
	 * List<CourseItem> l =
	 * UgFactory.getUgExam().getStudentExams(currentLoginObject,semester);
	 * return super.doInBackground(params); }
	 * 
	...
	static public void getAllExamsFromClient(final String userName, final String password, final Context context) {
	...
	static public void registrate(final String courseNumber,
			final String groupNumber, final String userName,
		...
		AsyncTask<Void, Void, Document> ast = new AsyncTask<Void, Void, Document>() {
			...
			protected void onPreExecute() {
				progressDialog = ProgressDialog.show(context,
						"Registering to course", "Trying to register to "
								+ courseNumber + " group " + groupNumber, true);
			...
			protected Document doInBackground(Void... arg0) {
				...
				try {
					...
					getCookiePost.setEntity(new UrlEncodedFormEntity(
							nameValuePairs));
					...
					Connection c = Jsoup
							.connect(
									"http://techmvs.technion.ac.il:100/cics/WMN/wmnnut02?OP=RS&RUTHY=RUTHY&Add+to+my+basket.x=12&Add+to+my+basket.y=18&LGRP1="
											...
											+ courseNumber
											+ "&LMK2=&LMK3=&RSND=")
							.timeout(7000).header("Cookie", cookie)
							.method(Method.GET);
					Document d = Jsoup.parse(new String(c.execute()
							.bodyAsBytes(), "ISO-8859-8"));
			...
			protected void onPostExecute(Document d) {
				...
				if (b) {
					...
					trackingCoursesFragment
							.onRegistrationSuccessed(new CourseKey(
									courseNumber, null));
				} else {
	...
	static public void unRegistrate(final int position, final String courseNumber,
		...
		AsyncTask<Void, Void, Document> ast = new AsyncTask<Void, Void, Document>() {
			...
			protected void onPostExecute(Document c) {
				...
				progressDialog.dismiss();
				alertDialogBuilder
						.setMessage("Course was removed successfully");
				alertDialogBuilder.show();
			}
