...
public class ClientEvent extends Event implements Parcelable {
  ...
  @Deprecated public ClientEvent() {
  }
  
  public ClientEvent(final long eventId, final String name, final String address, final String description, final long latitude,
      final long longitude, final EventDate when, final long invited, final EventType type, final String owner) {
    super(eventId, name, address, description, latitude, longitude, when, invited, type, owner);
  }
  ...
  public ClientEvent(final Parcel in) {
    super(in.readLong(), in.readString(), in.readString(), in.readString(), in.readLong(), in.readLong(), new EventDate(
        in.readLong()), in.readLong(), EventType.values()[in.readInt()], in.readString());
  ...
  @SuppressWarnings("rawtypes") public static final Parcelable.Creator<?> CREATOR = new Parcelable.Creator() {
    @Override public ClientEvent createFromParcel(final Parcel in) {
      return new ClientEvent(in);
    }
    
    @Override public ClientEvent[] newArray(final int size) {
