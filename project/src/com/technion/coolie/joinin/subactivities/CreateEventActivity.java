...
public class CreateEventActivity extends CoolieActivity {
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		setContentView(R.layout.ji_create_event);
		eventFBid = null;
	...
	private void updateListeners() {
		...
		descriptionInput.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(final CharSequence s, final int start,
				...
				charsCount.setTextColor(s.length() > MAX_DESCRIPTION ? Color
						.parseColor("#ff0000") : Color.parseColor("#333333"));
			}
...
//		 });
	...
	private void setupSpinner() {
		...
		catAdapter = new ArrayAdapter<CharSequence>(this,
				R.layout.ji_cat_spinner, R.id.rowTextView, data);
		...
		catSpinner.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(final AdapterView<?> parent,
					final View v, final int pos, final long row) {
	...
	private final TimePickerDialog.OnTimeSetListener timePickerListener = new TimePickerDialog.OnTimeSetListener() {
		@Override
		public void onTimeSet(final TimePicker view, final int selectedHour,
	...
	void pressOk(final ClientEvent e) {
		if (!checkParams())
			return;
		final ClientEvent toAdd = new ClientEvent(0, titleInput.getText()
				.toString().trim(), locationInput.getText().toString().trim(),
				descriptionInput.getText().toString().trim(), latitude,
				longtitude, date, invitedInput.getText().length() > 0 ? Long
						.valueOf(invitedInput.getText().toString()).longValue()
						: 0, type, mAccount.getUsername());
	...
	@Override
	protected void onActivityResult(final int arg0, final int arg1,
			final Intent arg2) {
...
//				hideProgressBar(importButton);
				...
				new DialogFragment() {
					...
					public Dialog onCreateDialog(final Bundle savedInstanceState) {
						final AlertDialog $ = new AlertDialog.Builder(
