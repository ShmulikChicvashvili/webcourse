	...
	protected void onHandleIntent(Intent arg0) {
		...
		case UG_GET_STUDENT_CURRENT_COURSES: {
			...
			student = new Gson().fromJson(
					(String) arg0.getSerializableExtra(INPUT), Student.class);
			semester = new Gson().fromJson(
					(String) arg0.getSerializableExtra(INPUT2), Semester.class);

			listOutput = UgFactory.getUgCourse().getStudentCurrentCourses(
					student, semester);
			intent.putExtra(STATUS, CoolieStatus.RESULT_OK);
			intent.putExtra(CoolieServerInterfaceService.RESULT,
					new Gson().toJson(listOutput));
		...
		case UG_GET_STUDENT_EXAMS: {
			...
			listOutput = UgFactory.getUgExam().getStudentExams(student,
					semester);
			intent.putExtra(STATUS, CoolieStatus.RESULT_OK);
			intent.putExtra(CoolieServerInterfaceService.RESULT,
					new Gson().toJson(listOutput));
		...
		case UG_GET_MY_GRADES_SHEET: {
			...
			// listOutput =
			// UgFactory.getUgGradeSheet().getMyGradesSheet(student);
			intent.putExtra(STATUS, CoolieStatus.RESULT_OK);
			// intent.putExtra(CoolieServerInterfaceService.RESULT,
			// new Gson().toJson(listOutput));
			sendBroadcast(intent);
		...
		case UG_ADD_TRACKING_STUDENT: {
			...
			student = new Gson().fromJson(
					(String) arg0.getSerializableExtra(INPUT), Student.class);
			courseKey = new Gson()
					.fromJson((String) arg0.getSerializableExtra(INPUT2),
							CourseKey.class);

			Output = UgFactory.getUgTracking().addTrackingStudent(student,
					courseKey);
			intent.putExtra(STATUS, CoolieStatus.RESULT_OK);
			intent.putExtra(CoolieServerInterfaceService.RESULT,
		...
		case WEBCOURSE_GET_STAFF_INFO: {
			...
			courseData = new Gson()
					.fromJson((String) arg0.getSerializableExtra(INPUT),
							CourseData.class);
		...
		case PARKION_USER_REPORT_PARKING_LOT_BUSY: {
			...
			Output = ParkionFactory.getParkionAPI().userReportParkingLotBusy(
					park);
			intent.putExtra(STATUS, CoolieStatus.RESULT_OK);
			intent.putExtra(CoolieServerInterfaceService.RESULT,
					new Gson().toJson(Output));
		...
		case TECHOINS_ADD_BANK_ACOUNTS: {
			...
			Output = TechoinsFactory.getTechoinsAPI().addAccount(account,
					password);
			intent.putExtra(STATUS, CoolieStatus.RESULT_OK);
			intent.putExtra(CoolieServerInterfaceService.RESULT,
					new Gson().toJson(Output));
		...
		case TECHOINS_GET_PRODUCTS_BY_NAME: {
			...
			product = new Gson().fromJson(
					(String) arg0.getSerializableExtra(INPUT), Product.class);

			OutputList = TechoinsFactory.getTechoinsAPI().getProductsByName(
					product);
			intent.putExtra(STATUS, CoolieStatus.RESULT_OK);
			intent.putExtra(CoolieServerInterfaceService.RESULT,
					new Gson().toJson(OutputList));
		...
		case TECHOINS_GET_SOLD_PRODUCTS_BY_SELLER_ID: {
			...
			OutputList = TechoinsFactory.getTechoinsAPI()
					.getSoldProductsBySellerID(product);
			intent.putExtra(STATUS, CoolieStatus.RESULT_OK);
			intent.putExtra(CoolieServerInterfaceService.RESULT,
					new Gson().toJson(OutputList));
			sendBroadcast(intent);
		...
		case TECHMINE_ADD_USER: {
			...
			user = new Gson().fromJson(
					(String) arg0.getSerializableExtra(INPUT), TecUser.class);
		...
		case TECHMINE_ADD_TOP_BEST_POST: {
			...
			com.technion.coolie.server.techmine.ReturnCode Output;
			Output = TechmineFactory.getTechmineAPI().addTopBestPost(
					topBestPost);
			intent.putExtra(STATUS, CoolieStatus.RESULT_OK);
			intent.putExtra(CoolieServerInterfaceService.RESULT,
					new Gson().toJson(Output));
		...
		case TECHMINE_REMOVE_TOP_BEST_POST: {
			...
			topBestPost = new Gson().fromJson(
					...
					TecTopBestPost.class);
			com.technion.coolie.server.techmine.ReturnCode Output;
			Output = TechmineFactory.getTechmineAPI().removeTopBestPost(
					topBestPost);
			intent.putExtra(STATUS, CoolieStatus.RESULT_OK);
			intent.putExtra(CoolieServerInterfaceService.RESULT,
					new Gson().toJson(Output));
		...
		case TECHMINE_ADD_TEC_POST: {
			TecPost tecPost;
			tecPost = new Gson().fromJson(
					(String) arg0.getSerializableExtra(INPUT), TecPost.class);
			com.technion.coolie.server.techmine.ReturnCode Output;
		...
		case TECHMINE_REMOVE_TEC_COMMENT: {
			...
			tecComment = new Gson()
					.fromJson((String) arg0.getSerializableExtra(INPUT),
							TecComment.class);
			com.technion.coolie.server.techmine.ReturnCode Output;
			Output = TechmineFactory.getTechmineAPI().removeTecComment(
					tecComment);
			intent.putExtra(STATUS, CoolieStatus.RESULT_OK);
			intent.putExtra(CoolieServerInterfaceService.RESULT,
					new Gson().toJson(Output));
		...
		case TECHMINE_ADD_TEC_LIKE: {
			TecLike tecLike;
			tecLike = new Gson().fromJson(
					(String) arg0.getSerializableExtra(INPUT), TecLike.class);
			com.technion.coolie.server.techmine.ReturnCode Output;
